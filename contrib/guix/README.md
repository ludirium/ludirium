# Bootstrappable Ludirium Core Builds

This directory contains the files necessary to perform bootstrappable Ludirium
Core builds.
...
To build Ludirium Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Ludirium Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/ludirium-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/ludirium-core/guix.sigs).
