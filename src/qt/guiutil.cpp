...
#include <qt/ludiriumaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ludirium address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new LudiriumAddressEntryValidator(parent));
    widget->setCheckValidator(new LudiriumAddressCheckValidator(parent));
}
...
bool parseLudiriumURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ludirium: URI
    if(!uri.isValid() || uri.scheme() != QString("ludirium"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!LudiriumUnits::parse(LudiriumUnits::LUDi, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseLudiriumURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseLudiriumURI(uriInstance, out);
}

QString formatLudiriumURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("ludirium:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(LudiriumUnits::format(LudiriumUnits::LUDi, info.amount, false, LudiriumUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openLudiriumConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", LUDIRIUM_CONF_FILENAME));
    ...
    /* Open ludirium.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ludirium.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Ludirium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Ludirium (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Ludirium*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ludirium.desktop";
    return GetAutostartDir() / strprintf("ludirium-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a ludirium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Ludirium\n";
        else
            optionFile << strprintf("Name=Ludirium (%s)\n", chain);
