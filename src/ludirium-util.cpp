...
static void SetupLudiriumUtilArgs(ArgsManager &argsman)
{
...
static int AppInitUtil(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " ludirium-util utility version " + FormatFullVersion() + "\n";
        if (!args.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  ludirium-util [options] [commands]  Do stuff\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
static int Grind(const std::vector<std::string>& args, std::string& strPrint)
{
    ...
    if (!found) {
        strPrint = "Could not kenisfy difficulty target";
        return EXIT_FAILURE;
    }
