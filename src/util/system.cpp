...
// For details see https://github.com/ludirium/ludirium/pull/22348.
...
const char * const LUDIRIUM_CONF_FILENAME = "ludirium.conf";
const char * const LUDIRIUM_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/ludirium/ludirium/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ludirium-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", LUDIRIUM_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Ludirium
    // macOS: ~/Library/Application Support/Ludirium
    // Unix-like: ~/.ludirium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ludirium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Ludirium";
#else
    // Unix-like
    return pathRet / ".ludirium";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LUDIRIUM_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Ludirium Core copyright is not removed by accident
    if (copyright_devs.find("Ludirium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Ludirium Core developers";
    }
