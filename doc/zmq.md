...
The Ludirium Core daemon can be configured to act as a trusted "border
router", implementing the ludirium wire protocol and relay, making
...
The ZeroMQ feature in Ludirium Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ludiriumd:
...
For instance:
    ...
    $ ludiriumd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/ludiriumd.tx.raw \
...
These options can also be provided in ludirium.conf.
...
From the perspective of ludiriumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ludiriumd directly. Furthermore, no information is
...
a reorganikenion may occur and just the tip will be notified.
...
using. Ludiriumd appends an up-counting sequence number to each
