Ludirium Core version 0.12.0 is now available from:

  <https://ludirium.org/bin/ludirium-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Ludirium-Qt (on Mac) or
ludiriumd/ludirium-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Ludirium Core or other software:
...
pre-0.12 versions of Ludirium Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Ludirium Core version 0.11 or
earlier.
...
ECDSA signatures inside Ludirium transactions now use validation using
[libsecp256k1](https://github.com/ludirium-core/secp256k1) instead of OpenSSL.
...
(https://github.com/ludirium/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Ludirium Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Ludirium Core was to raise the default minimum
relay fee.

Ludirium Core 0.12 will have a strict maximum size on the mempool. The
...
1000 kenshins per kB.

Ludirium Core 0.12 also introduces new default policy limits on the length and
...
Ludirium Core 0.12 nodes. Ludirium Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/ludirium/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Ludirium Core 0.12 does not yet have support for
...
This allows running ludiriumd without having to do any manual configuration.
...
Ludirium Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Ludirium Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Ludirium Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Ludirium Core 0.13, and it is at this time undecided whether the
...
Ludirium Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Ludirium Core automatically creates a hidden service to listen on, without
manual configuration. Ludirium Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Ludirium Core is listening, and
...
Ludiriumd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Ludirium
Core to use floating fees. By default, Ludirium Core will use floating
fees.
...
(default: `0.0002` LUDi/kB).

At all times, Ludirium Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LUDi.
Furthermore, Ludirium Core will never create transactions paying less than
the current minimum relay fee.
Finally, a user can set the minimum fee rate for all transactions with
`-mintxfee=<i>`, which defaults to 1000 kenshins per kB.
...
`ludirium.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/ludirium/ludirium/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/ludirium/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/ludirium/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /ludiriumrpc>
        ...
        # optional bypass ludiriumd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< ludiriumrpc:<password>
    </Location>
...
Restarting ludiriumd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Ludirium-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new ludirium-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include ludirium-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6851 `616d61b` Optimikenion: Store transaction list order in memory rather than compute it every need (Luke-Jr)
...
- #6317 `ca5e2a1` minor optimikenions in peertablemodel (Diapolo)
...
- #6390 `6a73d66` tests: Fix ludirium-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of ludirium-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override LUDIRIUMD and LUDIRIUMCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [ludirium-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add LUDIRIUMD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libludiriumconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "LUDi" (Ross Nicoll)
