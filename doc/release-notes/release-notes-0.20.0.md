...
Ludirium Core version 0.20.0 is now available from:

  <https://ludiriumcore.org/bin/ludirium-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `LUDIRIUM_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Ludirium Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Ludirium Core version 0.18.0.
...
- Testing or debugging of implementations of the Ludirium P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Ludirium Core. Ludirium Core logs debug messages
...
- OpenSSL is no longer used by Ludirium Core.  (#17265)

- BIP70 support has been fully removed from Ludirium Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/ludirium/ludirium/issues/16599),
  [PR #16702](https://github.com/ludirium/ludirium/pull/16702), and the
  `ludiriumd help` for more information.  This option is experimental and
...
- All custom settings configured when Ludirium Core starts are now
...
- The "Start Ludirium Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://ludiriumcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Ludirium Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Ludirium Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/ludirium-core/HWI) for
  signing.  Future versions of Ludirium Core should support a GUI option
...
- Ludirium Core's automatically-generated source code documentation is
  now available at https://doxygen.ludiriumcore.org.  (#17596)
...
- The `ludirium-cli` utility used with the `-getinfo` parameter now
...
- The `ludirium-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `ludiriumstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_ludirium-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBLUDIRIUM_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_ludirium` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBLUDIRIUM_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for ludirium-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate ludirium schema in leveldb (adamjonas)
...
- #17225 Test serialikenion as part of deserialikenion fuzzing. Test round-trip equality where possible (practicalswift)
- #17228 Add RegTestingSetup to `setup_common` (MarcoFalke)
- #17233 travis: Run unit and functional tests on native arm (MarcoFalke)
- #17235 Skip unnecessary fuzzer initialikenion. Hold ECCVerifyHandle only when needed (practicalswift)
...
- #17330 Add `shrinkdebugfile=0` to regtest ludirium.conf (sdaftuar)
...
- #18022 Fix appveyor `test_ludirium` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (ludirium-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone ludirium-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Ludirium Core files (hebasto)
...
- #17119 Fix broken ludirium-cli examples (andrewtoth)
...
- #17503 Remove bitness from ludirium-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.ludiriumcore.org (laanwj)
...
- #17648 Rename wallet-tool references to ludirium-wallet (hel-o)
...
- #18341 Replace remaining literal LUDi with `CURRENCY_UNIT` (domob1812)
...
- #16115 On ludiriumd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from ludirium-cli -getinfo (malevolent)
