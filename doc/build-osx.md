...
This guide describes how to build ludiriumd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Ludirium Core from source.
...
### 4. Clone Ludirium repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Ludirium Core repository to a directory.
...
It is not necessary to build wallet functionality to run `ludiriumd` or  `ludirium-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/ludirium/ludirium/blob/master/doc/descriptors.md), `sqlite` is required.
...
Ludirium Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/ludirium/ludirium/issues/7714).
...
You can deploy a `.dmg` containing the Ludirium Core application using `make deploy`.
...
Run the following in your terminal to compile Ludirium Core:
...
## Running Ludirium Core

Ludirium Core should now be available at `./src/ludiriumd`.
If you compiled support for the GUI, it should be available at `./src/qt/ludirium-qt`.

The first time you run `ludiriumd` or `ludirium-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Ludirium/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Ludirium"

touch "/Users/${USER}/Library/Application Support/Ludirium/ludirium.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Ludirium/ludirium.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Ludirium/debug.log
```

## Other commands:

```shell
./src/ludiriumd -daemon      # Starts the ludirium daemon.
./src/ludirium-cli --help    # Outputs a list of command-line options.
./src/ludirium-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/ludirium-qt -server # Starts the ludirium-qt server mode, allows ludirium-cli control
```
