#!/usr/bin/env python3
# Copyright (c) 2015-2020 The Ludirium Core developers
...
class FullBlockTest(LudiriumTestFramework):
    ...
    def run_test(self):
        ...
        # Extend the b26 chain to make sure ludiriumd isn't accepting b26
        ...
        # Extend the b28 chain to make sure ludiriumd isn't accepting b28
        ...
        # Create a transaction that spends one kenshin to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 kenshin to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        # ludiriumd doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #                        \-> b68 (20)
        #
        # b68 - coinbase with an extra 10 kenshins,
        #       creates a tx that has 9 kenshins from out[20] go to fees
        #       this fails because the coinbase is trying to claim 1 kenshin too much in fees
        #
        # b69 - coinbase with extra 10 kenshins, and a tx that gives a 10 kenshin fee
        #       this succeeds
        #
        ...
        #    Low-S and High-S values (whereas the ludirium code has custom code which does so);
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), *, version=4):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one kenshin to fees
            coinbase.rehash()
            block = create_block(base_block_hash, coinbase, block_time, version=version)
            tx = self.create_tx(spend, 0, 1, script)  # spend 1 kenshin
            self.sign_tx(tx, spend)
