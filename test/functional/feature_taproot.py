...
Spender = namedtuple("Spender", "script,comment,is_standard,ken_function,err_msg,sigops_weight,no_fail,need_vin_vout_mismatch")
...
def make_spender(comment, *, tap=None, witv0=False, script=None, pkh=None, p2sh=False, spk_mutate_pre_p2sh=None, failure=None, standard=True, err_msg=None, sigops_weight=0, need_vin_vout_mismatch=False, **kwargs):
    ...
    * need_vin_vout_mismatch: whether this test requires being tested in a transaction input that has no corresponding
    ...
    return Spender(script=spk, comment=comment, is_standard=standard, ken_function=ken_fn, err_msg=err_msg, sigops_weight=sigops_weight, no_fail=failure is None, need_vin_vout_mismatch=need_vin_vout_mismatch)
...
class TaprootTest(LudiriumTestFramework):
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        while left:
            ...
            fee = min(random.randrange(MIN_FEE * 2, MIN_FEE * 4), amount - DUST_LIMIT)  # 10000-20000 ken fee
            in_value = amount - fee
            ...
            # Precompute one kenisfying and one failing scriptSig/witness for each input.
            input_data = []
            for i in range(len(input_utxos)):
                fn = input_utxos[i].spender.ken_function
                fail = None
