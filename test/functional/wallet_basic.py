...
class WalletTest(LudiriumTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 LUDi from 0 to 2 using sendtoaddress call.
        ...
        # Send 10 LUDi with subtract fee from amount
        ...
        # Sendmany 10 LUDi with subtract fee from amount
        ...
        self.log.info("Test sendmany with fee_rate param (explicit fee rate in ken/vB)")
        fee_rate_ken_vb = 2
        fee_rate_btc_kvb = fee_rate_ken_vb * 1e3 / 1e8
        explicit_fee_rate_btc_kvb = Decimal(fee_rate_btc_kvb) / 1000

        # Test passing fee_rate as a string
        txid = self.nodes[2].sendmany(amounts={address: 10}, fee_rate=str(fee_rate_ken_vb))
        self.nodes[2].generate(1)
        self.sync_all(self.nodes[0:3])
        ...
        txid = self.nodes[2].sendmany(amounts={address: amount}, fee_rate=fee_rate_ken_vb)
        self.nodes[2].generate(1)
        self.sync_all(self.nodes[0:3])
        balance = self.nodes[2].getbalance()
        ...
        assert_raises_rpc_error(-6, "Fee rate (0.999 ken/vB) is lower than the minimum fee rate setting (1.000 ken/vB)",
        ...
        msg = "Fee rate (0.000 ken/vB) is lower than the minimum fee rate setting (1.000 ken/vB)"
        ...
        # Test fee_rate values that cannot be represented in ken/vB.
        ...
        for target, mode in product([-1, 0], ["btc/kb", "ken/b"]):
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Ludirium address
            assert_raises_rpc_error(-5, "Invalid Ludirium address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Ludirium address
            assert_raises_rpc_error(-5, "Invalid Ludirium address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Ludirium address or script", self.nodes[0].importaddress, "invalid")
            ...
            assert_raises_rpc_error(-5, "Bech32m addresses cannot be imported into legacy wallets", self.nodes[0].importaddress, "lurt1p0xlxvlhemja6c4dqv22uapctqupfhlxm9h8z3k2e72q4k9hcz7vqc8gma6")
            ...
            self.log.info("Test sendtoaddress with fee_rate param (explicit fee rate in ken/vB)")
            ...
            fee_rate_btc_kvb = fee_rate_ken_vb * 1e3 / 1e8
            # Test passing fee_rate as an integer
            txid = self.nodes[2].sendtoaddress(address=address, amount=amount, fee_rate=fee_rate_ken_vb)
            ...
            fee_rate_ken_vb = 1.23
            fee_rate_btc_kvb = fee_rate_ken_vb * 1e3 / 1e8
            # Test passing fee_rate as a string
            txid = self.nodes[2].sendtoaddress(address=address, amount=amount, fee_rate=str(fee_rate_ken_vb))
            tx_size = self.get_vsize(self.nodes[2].gettransaction(txid)['hex'])
            self.nodes[0].generate(1)
            self.sync_all(self.nodes[0:3])
            postbalance = self.nodes[2].getbalance()
