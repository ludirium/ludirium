...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s LUDi too low! (Should be %s LUDi)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s LUDi too high! (Should be %s LUDi)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting LUDi values"""
    n = Decimal("20000000.00000003")
    kenshins = int(json.loads(json.dumps(float(n))) * 1.0e8)
    if kenshins != 2000000000000003:
...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `LudiriumTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "ludirium.conf"), n=n, chain=chain)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "ludirium.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "ludirium.conf")):
        with open(os.path.join(datadir, "ludirium.conf"), 'r', encoding='utf8') as f:
            for line in f:
...
def create_confirmed_utxos(fee, node, count):
    ...
    for _ in range(iterations):
        ...
        outputs[addr1] = kenshin_round(send_value / 2)
        outputs[addr2] = kenshin_round(send_value / 2)
...
def chain_transaction(node, parent_txids, vouts, value, fee, num_outputs):
    ...
    """
    send_value = kenshin_round((value - fee)/num_outputs)
    inputs = []
...
def create_lots_of_big_transactions(node, txouts, utxos, num, fee):
    ...
    for _ in range(num):
        ...
        outputs = {}
        change = t['amount'] - fee
        outputs[addr] = kenshin_round(change)
