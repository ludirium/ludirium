...
from test_framework.util import assert_equal, assert_raises_rpc_error, kenshin_round
...
def make_utxo(node, amount, confirmed=True, scriptPubKey=DUMMY_P2WPKH_SCRIPT):
    ...
    """
    fee = 1 * COIN
    while node.getbalance() < kenshin_round((amount + fee) / COIN):
        node.generate(COINBASE_MATURITY)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, kenshin_round((amount + fee) / COIN))
...
class ReplaceByFeeTest(LudiriumTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 LUDi - so this attempt is rejected.
        dbl_tx = CTransaction()
    ...
    def test_replacement_relay_fee(self):
        ...
        # fee conforming to node's `incrementalrelayfee` policy of 1000 ken per KB.
        tx.vout[0].nValue -= 1
